...
# # STEP 7: SOLVE THE QUBO USING QAOA (Quantum Algorithm)
# # =============================================================================
# # Convert the QUBO dictionary Q into a QuadraticProgram
# qp = QuadraticProgram()
# for i in range(num_vars):
#     qp.binary_var(name=f'x_{i}')
...
# # Extract the solution as a list of 0s and 1s
# solution = [int(result.x[i]) for i in range(num_vars)]
# print(f"Best energy found (QAOA): {result.fval}")
...
# # =============================================================================
# # NEW Subplot 6: Distribution of Handlers by Skill Level and Severity
# # (Summed Across All Clusters)
# # =============================================================================
# # Aggregate the staffing_solution across all clusters for each (skill, severity)
# skill_severity_counts = {}
# for (s, j, c), val in staffing_solution.items():
#     skill_severity_counts[(s, j)] = skill_severity_counts.get((s, j), 0) + val

# # Create a structure: severity on X-axis, stacked bars for skill=1..5
# severity_sorted = sorted(severity_levels)
# skill_levels = [1, 2, 3, 4, 5]

# # Build a 2D list: rows = severity, columns = skill
# counts_by_sev_and_skill = []
# for j in severity_sorted:
#     row = []
#     for s in skill_levels:
#         row.append(skill_severity_counts.get((s, j), 0))
#     counts_by_sev_and_skill.append(row)

# ax5 = fig.add_subplot(gs[3, :])
# x_indices = np.arange(len(severity_sorted))
# bottom_vals = np.zeros(len(severity_sorted))
# for skill_idx, s in enumerate(skill_levels):
#     current_counts = [counts_by_sev_and_skill[j_idx][skill_idx] for j_idx in range(len(severity_sorted))]
#     ax5.bar(x_indices, current_counts, bottom=bottom_vals, label=f'Skill {s}')
#     bottom_vals += np.array(current_counts)

# ax5.set_title("Distribution of Handler Skills by Claim Severity (Summed Across Clusters)")
# ax5.set_xlabel("Severity")
# ax5.set_ylabel("Number of Handlers")
# ax5.set_xticks(x_indices)
# ax5.set_xticklabels([str(j) for j in severity_sorted])
# ax5.legend()
